...
(function() {
  ...
  module.run([
    '$templateCache', function($templateCache) {
      return $templateCache.put('contrib/iframe-show.html', '<div class="wrapper"><tg-project-menu></tg-project-menu><section class="main iframe"><h1> <a href="{{iframe.url}}">{{iframe.title}}</a></h1><iframe src="{{trustSrc(iframe.url)}}" name="{{iframe.slug}}" style="width:100%; height: 100%"></iframe></section></div>');
    }
  ]);
