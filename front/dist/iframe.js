...
(function() {
  ...
  module.run([
    '$templateCache', function($templateCache) {
      return $templateCache.put('contrib/iframe', '<div ng-controller="ContribIframeAdminController as ctrl"><header><h1><span class="project-name">{{::project.name}}</span><span class="green">{{::sectionName}} </span></h1></header><section class="project-iframe-admin"><div tg-project-values="tg-project-values" type="userstory-statuses" ng-controller="ProjectValuesController as ctrl" ng-init="section=\'admin\'; resource=\'iframe\'; type=\'userstory-statuses\'; sectionName=\'ADMIN.PROJECT_VALUES_STATUS.US_TITLE\'" objName="status" class="admin-attributes-section"><div class="project-values-title"><h2>Iframes</h2><a href="" title="Add Iframe" class="button button-gray show-add-new"><span>Add Iframe</span></a></div><div class="colors-table"><div class="table-header"><div class="row"><div style="flex-basis: 60px; flex-grow: 0; max-width: 100px; position: relative" class="iframe-icon">Icon</div><div style="flex-basis: 60px; flex-grow: 1; position: relative" class="iframe-title">Title</div><div style="flex-basis: 60px; flex-grow: 1;  position: relative" class="iframe-slug">Slug</div><div style="padding-left: 20px; flex-basis: 120px; flex-grow: 3; position: relative" class="iframe-url">URL</div><div style="padding-left: 45px; flex-basis: 250px; flex-grow: 6; position: relative" class="iframe-html">HTML</div><div class="options-column"></div></div></div><div class="table-main"><div class="sortable ui-sortable"><div ng-repeat="value in values" tg-bind-scope="tg-bind-scope" class="tg-scope"><form tg-bind-scope="tg-bind-scope" style="max-width:100%"><div class="row table-main visualization"><span class="icon icon-drag-v"></span><div style="flex-basis: 60px; flex-grow: 0; max-width:100px; position: relative" class="iframe-icon"><span class="icon"><img ng-src="{{value.icon}}"/></span></div><div style="flex-basis: 60px; flex-grow: 1; position: relative" class="iframe-title"><span>{{ value.title }}</span></div><div style="flex-basis: 60px; flex-grow: 1; position: relative" class="iframe-slug"><span>{{ value.slug }}</span></div><div style="flex-basis: 120px; flex-grow: 3; position: relative" class="iframe-url"><span>{{ value.url }}</span></div><div style="flex-basis: 250px; flex-grow: 6; position: relative" class="iframe-html"><span>{{ value.html }}</span></div><div class="options-column"><a href="" title="{{\'ADMIN.COMMON.TITLE_ACTION_EDIT_VALUE\' | translate}}" class="a edit-value icon icon-edit"></a><a href="" title="{{\'ADMIN.COMMON.TITLE_ACTION_DELETE_VALUE\' | translate}}" class="delete-value icon icon-delete"></a></div></div><div class="row table-main edition hidden"><div style="flex-basis: 60px; flex-grow: 0; max-width:100px; position: relative" class="iframe-icon"><span>FIXME</span></div><div style="flex-basis: 60px; flex-grow: 1; position: relative" class="iframe-title"><input name="title" type="text" placeholder="Title" ng-model="value.title" data-required="true" data-maxlength="255"/></div><div style="flex-basis: 60px; flex-grow: 1; position: relative" class="iframe-slug"><input name="slug" type="text" placeholder="Slug" ng-model="value.slug" data-required="true" data-maxlength="255"/></div><div style="flex-basis: 120px; flex-grow: 3; position: relative" class="iframe-url"><input name="url" type="text" placeholder="http://URL" ng-model="value.url" data-required="true" data-maxlength="255"/></div><div style="flex-basis: 250px; flex-grow: 6; position: relative" class="iframe-html"><input name="html" type="text" placeholder="HTML" ng-model="value.html" data-required="false" data-maxlength="8092"/></div><div class="options-column"><a href="" title="{{\'COMMON.SAVE\' | translate}}" class="save icon icon-floppy"></a><a href="" title="{{\'COMMON.CANCEL\' | translate}}" class="cancel icon icon-delete"></a></div></div></form></div></div></div></div></div></section></div>');
    }
  ]);
