...
(function() {
  var IFRAMES_STUB, Iframe, IframeAdmin, IframeInfo, decorator, decoratorInfo, initIframePlugin, module;
  ...
  decorator = function($delegate, $templateCache) {
      ...
      this.scope.$on("project:loaded", (function(_this) {
        return function() {
          ...
          return promise["catch"](function(e) {
            return scope.iframes = IFRAMES_STUB;
          });
        };
      })(this));
    }
  ...
  Iframe = (function() {
    Iframe.$inject = ["$scope", "$tgRepo", "$route", "$sce"];

    function Iframe(scope1, repo, route, $sce) {
      var iframe, scope, slug;
      this.scope = scope1;
      this.repo = repo;
      this.route = route;
      scope = this.scope;
  ...
  module.run([
    '$templateCache', function($templateCache) {
      return $templateCache.put('contrib/iframe', '<div ng-controller="ContribIframeAdminController as ctrl"><header><h1><span class="project-name">{{::project.name}}</span><span class="green">{{::sectionName}} </span></h1></header><div class="project-values-title"><h2>Iframes</h2><a href="" title="Add Iframe" class="button button-gray show-add-new"><span>Add Iframe</span></a></div><div class="colors-table"><div class="table-header"><div class="row"><div class="color-column">Icon</div><div class="status-name">Title</div><div class="status-slug">Slug</div><div class="status-slug"> URL</div><div class="options-column"></div></div></div><div class="table-main"><div class="sortable"><div ng-repeat="i in iframes" tg-bind-scope="tg-bind-scope" class="tg-scope"><form tg-bind-scope="tg-bind-scope"><div class="row table-main visualization"><span class="icon icon-drag-v"></span><div class="color-column"><span class="icon"><img ng-src="data:image/png;base64, {{i.icon}}"/></span></div><div class="status-name"><span>{{ i.title }}</span></div><div class="status-slug"><span>{{ i.slug }}</span></div><div class="status-slug"><span>{{ i.url }}</span></div><div class="options-column"><a href="" title="{{\'ADMIN.COMMON.TITLE_ACTION_EDIT_VALUE\' | translate}}" class="edit-value icon icon-edit"></a><a href="" title="{{\'ADMIN.COMMON.TITLE_ACTION_DELETE_VALUE\' | translate}}" class="delete-value icon icon-delete"></a></div></div><div class="row table-main edition hidden"><div class="options-column"><a href="" title="{{\'COMMON.SAVE\' | translate}}" class="save icon icon-floppy"></a><a href="" title="{{\'COMMON.CANCEL\' | translate}}" class="cancel icon icon-delete"></a></div></div></form></div></div></div></div></div>');
    }
  ]);

  module.run([
    '$templateCache', function($templateCache) {
      return $templateCache.put('contrib/iframe-menu.html', '<nav ng-if="vm.project" class="menu"><div class="menu-container"><ul class="main-nav"><li ng-repeat="i in iframes" id="{{i.slug}}"><a tg-nav="project-iframe:project=vm.project.get(\'slug\'),iframe=i.slug" ng-class="{active: vm.active == \'iframe\'}" title="{{i.title}}"><span class="icon"><img ng-src="data:image/png;base64, {{i.icon}}"/></span><span class="helper">{{i.title}}</span></a></li></ul></div></nav>');
    }
  ]);
